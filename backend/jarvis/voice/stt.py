    ...
    def _load(self, model_name: str, device: str) -> None:
        ...
        if device == "auto":
            try:
                ...
                resolved_device = "cpu"
        compute = "float16" if resolved_device == "cuda" else "int8"
        log.info("Lade Whisper-Modell %s (%s, %s)", model_name, resolved_device, compute)
        try:
            self._model = WhisperModel(model_name, device=resolved_device, compute_type=compute, download_root=str(MODEL_DIR))
